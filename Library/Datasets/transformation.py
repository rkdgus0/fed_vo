...
def pose2motion(data, skip=0):
    ...
    for i in range(data_size - 1 - skip):
...
def sos2quats(so_datas):
    ...
    for i_b in range(so_datas.shape[0]):
        for i_p in range(so_datas.shape[1]):
...
def SO2quat(SO_data):
    return R.from_matrix(SO_data).as_quat()
